...
if ($database->executeStatement_numrow("SELECT * FROM `menu` WHERE `menu_name` = '$menu_item' and `menu_id` != '$menu_item_id' LIMIT 1 ") > 0) {
...
if (isset($_FILES['myfile'])) {
    ...
    if (in_array(findexts($sFileName), $allowed_mime)) {
        ...
        if (move_uploaded_file($tempFile, $pathname)) {
            ...
            $database->executeStatement("UPDATE menu SET menu_cate_id = '$menu_item_cate', menu_name = '$menu_item', menu_price = '$menu_item_price', menu_img = '$filename2', menu_status = '$menu_item_status' WHERE menu_id='$menu_item_id' ");
            echo <<<EOF
...
EOF;
    ...
    $database->executeStatement("UPDATE menu SET menu_cate_id = '$menu_item_cate', menu_name = '$menu_item', menu_price = '$menu_item_price' , menu_status = '$menu_item_status' WHERE menu_id='$menu_item_id' ");
    echo 'OK';
}

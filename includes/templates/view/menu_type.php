...
$txt_search = mysql_real_escape_string(trim(isset($_REQUEST['search_txt']) ? $_REQUEST['search_txt']: NULL));
$type_search = mysql_real_escape_string(trim(isset($_REQUEST['search_type']) ? $_REQUEST['search_type']:NULL));
...
$sql_menu = "SELECT * FROM (SELECT `menu_category`.*, count(menu_id) as count_menu FROM `menu_category` 
    ...
    GROUP BY `menu_category`.menu_cate_id  ORDER BY `menu_cate_id` ASC) AS menu_category  " . (isset($type_search_arr[$type_search]) ? $type_search_arr[$type_search]: NULL) . "   ";

    ...
    <body style="background-color: #E7E4E4">
                ...
                <div class="container">
                    <ul class="nav navbar-nav">
                        ...
                        if (in_array($sys_user_role, array(1, 2, 3))) {  // cook,cashier,owner
                            echo '<li class="active"><a href="kitchen.php" style="color: #990000" > <b>Kitchen</b></a></li>';
                        }
                        if (in_array($sys_user_role, array(1, 2, 4))) {  // waiter,cashier,owner
        ...
        <script>
            window.setInterval(function() {
                ...
                $.post("ajax/ajax_load_more_kitchen.php", {}, function(data) {
                    $("#monitor_table").html(data);
                });
            }, 5000);
            $(function() {
                $("#box_monitor").on("click", ".btn-kitchen", function() {
                    var this_item = $(this);
                    ...
                    $.post("ajax/ajax_order_status_process.php", {btn: btn_type, id: food_id}, function(data) {
                        ...
                        if (data.result == "ok") {
                            if (btn_type == "cook") {
                                this_item.hide();
                                this_item.parent().prepend('<button type="button" class="btn btn-primary btn-kitchen" data-id="cook_finish">Finish</button>');
                                this_item.remove();
                            } else {
